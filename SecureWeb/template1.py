...
class HomeHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        str1=cgi.escape(username).encode('ascii', 'xmlcharrefreplace')
        con = lite.connect('db.db')
        with con:
            cur = con.cursor()
            cur.execute("SELECT count(*) FROM tag WHERE username= %s and password= %s" %("'"+str1+"'","'"+password+"'"))
            x=cur.fetchone()[0]
            if x!=0:
                self.write('<html><body bgcolor="#E6E6FA"><p>Welcome ' + str1 + '</p>'
...
class SearchHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        username=self.get_argument("username")
        str2=cgi.escape(username).encode('ascii', 'xmlcharrefreplace')
        self.write('<html><body><p>Welcome '+str2+ '</p></body></html>')
...
class SignupXss(tornado.web.RequestHandler):
    ...
    def post(self):
        username=self.get_argument("username")
        password=self.get_argument("password") 
        name=cgi.escape(username).encode('ascii', 'xmlcharrefreplace')               
        con = lite.connect('db.db')
        with con:
            cur = con.cursor()
            cur.execute("INSERT INTO tag VALUES(?,?)",(name,password))
...
class ListuserXss(tornado.web.RequestHandler):
    def get(self):
        ...
        with con:
            ...
            for l1 in x:
                l2=cgi.escape(l1[0]).encode('ascii', 'xmlcharrefreplace')
                self.write(l2) 
